...
public class AutenticacionControlador {
    ...
    public Usuarios iniciarSesion(String email, String password, JFrame vistaLogin) {
        ...
        Usuarios loginUser = new Usuarios(0, "", "", null, "", email, "", password, 0); // 1. Creamos un usuario
                                                                                        // temporal con las credenciales
        MensajeRed peticion = new MensajeRed("LOGIN", loginUser, true, "");// 2. Empaquetamos en el MensajeRed que ya
                                                                           // definiste

        MensajeRed respuesta = clienteSocket.getInstancia().enviarPeticion(peticion); // 3. Enviamos a través de tu
                                                                                      // clase ClienteSocket (usando
                                                                                      // Singleton)
